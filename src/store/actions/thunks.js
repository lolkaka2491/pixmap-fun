...
export function fetchChatMessages(cid) {
  return async (dispatch) => {
    ...
    const response = await requestChatMessages(cid);
    if (response && response.history) {
      setTimeout(() => { dispatch(setChatFetching(false)); }, 500);
      dispatch(receiveChatHistory(cid, response.history, response.reactions || {}));
    } else {
      setTimeout(() => { dispatch(setChatFetching(false)); }, 1000);
      dispatch(receiveChatHistory(cid, [], {}));
    }
  };
}
