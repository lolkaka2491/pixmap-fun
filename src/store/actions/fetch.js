...
async function makeAPIPOSTRequest(
  ...
  try {
    console.log('About to fetch:', url, body);
...
export async function requestChatMessages(cid) {
  ...
  const limit = state.gui.chatHistoryLength;
  const response = await fetch(
    `${shardOrigin}/api/chathistory?cid=${cid}&limit=${limit}`,
    { credentials: 'include' },
  );
  // timeout in order to not spam api requests and get rate limited
  if (response.ok) {
    const data = await response.json();
    return data; // Return full response object instead of just history
  }
  return null;
}
...
export async function requestLogin(nameoremail, password) {
  try {
    ...
    if (!response.ok) {
      return {
        errors: [ json.message || 'Login failed' ],
        me: null,
      };
    }
...
export function requestBanInfo(url = '/api/baninfo') {
  return makeAPIGETRequest(
    url,
  );
}
...
export async function requestMe() {
  ...
  try {
    // lazy import to avoid client/server crossover issues
    const { __getCanvasTokenHeaderForFetch } = require('../../socket/SocketClient');
    extraHeaders = __getCanvasTokenHeaderForFetch();
  } catch {}
  return makeAPIGETRequest(
    '/api/me',
    true,
