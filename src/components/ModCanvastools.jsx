...
import { shardOrigin, requestHistoricalTimes } from '../store/actions/fetch';
...
async function submitCanvasCleaner(
  ...
  if (action === 'replacecolor') {
    data.append('sourcecolor', sourceColor);
...
function ModCanvastools() {
  ...
  const [scheduleRollback, setScheduleRollback] = useState(false);
  const [scheduleProtect, setScheduleProtect] = useState(false);
  const [scheduleDate, setScheduleDate] = useState('');
  const [scheduleTime, setScheduleTime] = useState('00:00');
  const [sourceColor, setSourceColor] = useState(0);
  ...
  useEffect(() => {
    if (date) {
      requestHistoricalTimes(date, selectedCanvas).then((newTimes) => {
  ...
  switch (cleanAction) {
    ...
    case 'makeblank':
      // eslint-disable-next-line max-len
      descCleanAction = t`Turn every pixel in area to white`;
      break;
    case 'replacecolor':
      // eslint-disable-next-line max-len
      descCleanAction = t`Replace all pixels of one color with another color in the defined area`;
      break;
    default:
      // nothing
  }
    ...
    <div className="content">
      ...
      {(window.ssv && window.ssv.backupurl) && (
        <div>
          <br />
          <div className="modaldivider" />
          <h3>{t`Rollback to Date and Time`}</h3>
          <p>
            {t`Rollback an area of the canvas to a specific date and time`}
          </p>
          <div className="historyselect">
            ...
            <div className="timeselcon">
              ...
              {!times.length && <p>{t`Select Date above`}</p>}
            </div>
          </div>
          <p>
            {t`Top-left corner`}:&nbsp;
      ...
      >
        {['spare', 'spareext', 'spareextu', 'makenull', 'makeblank', 'replacecolor'].map((opt) => (
          <option
            key={opt}
            value={opt}
          >
      ...
      <p>{descCleanAction}</p>
      {cleanAction === 'replacecolor' && (
        <div>
          <p>
            {t`Source Color (to replace)`}:&nbsp;
            <select
              value={sourceColor}
              onChange={(e) => setSourceColor(parseInt(e.target.value, 10))}
              style={{ minWidth: '200px' }}
            >
              {canvases[selectedCanvas] && canvases[selectedCanvas].colors.map((color, index) => (
                <option 
                  key={`source-${index}`} 
                  value={index}
                  style={{ 
                    backgroundColor: `rgb(${color[0]}, ${color[1]}, ${color[2]})`,
                    color: (color[0] + color[1] + color[2]) > 384 ? 'black' : 'white',
                    padding: '4px'
                  }}
                >
          ...
          <p>
            {t`Target Color (replace with)`}:&nbsp;
            <select
              value={targetColor}
              onChange={(e) => setTargetColor(parseInt(e.target.value, 10))}
              style={{ minWidth: '200px' }}
            >
              {canvases[selectedCanvas] && canvases[selectedCanvas].colors.map((color, index) => (
                <option 
      ...
      <button
        ...
        onClick={() => {
          ...
          submitCanvasCleaner(
            ...
            cleanAction === 'replacecolor' ? sourceColor : undefined,
            cleanAction === 'replacecolor' ? targetColor : undefined,
          );
        }}
      >
        {(submitting) ? '...' : t`Submit`}
