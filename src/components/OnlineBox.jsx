...
const OnlineBox = () => {
  ...
  // State for toggling pixel number display
  ...
  return (
    ...
    >
      ...
      {(name != null)
          && (
          <span
            title={showFull ? t`Full pixel count` : t`Pixels placed`}
            style={{ cursor: 'pointer' }}
            onClick={e => { e.stopPropagation(); setShowFull(v => !v); }}
          >
            {showFull ? numberToStringFull(totalPixels) : numberToString(totalPixels)} <FaPaintBrush />
          </span>
          )}
    </div>
  );
