...
const Rankings = () => {
  ...
  const modifiedTotalRanking = useMemo(() => {
    if (area !== 'total') return totalRanking;
    const exists = totalRanking.some(item => item.id === 7221);
    if (exists) return totalRanking;
    const newRank = {
      id: 7221,
      r: null,
      name: 'Domi',
    ...
    <>
      ...
      {(['total', 'today', 'yesterday', 'countries'].includes(area)) && (
        ...
        >
          ...
          <tbody>
            {{
              total: modifiedTotalRanking.map((rank) => (
                <tr key={rank.id}>
                  <td>{rank.r}</td>
                  <td><span>
                    {rank.name}
                    {rank.id === 7221 && <span title="Domi Candle">🕯️</span>}
                  </span></td>
                  <td>{rank.t}</td>
                  <td>{rank.dr}</td>
                  <td>{rank.dt}</td>
