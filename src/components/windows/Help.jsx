...
/** Creates a button that looks like an action button.
 * However, the button does not have any functionality, and can be used inside a paragraph.
 * @param {*} children - The image of the button is expected to be passed in as the child.
 * @returns A `<span>` element with the class ".actionbuttons" and the passed in child(ren).
 */
function ActionButtonWrapper({ children }) {
  return (
    <span 
      className='actionbuttons'
      style={{padding: '0.5em', position: 'relative', verticalAlign: 'middle'}}
    >
      {children}
    </span>
  );
...
const Help = () => {
  ...
  const mailLink = <a href="mailto:chninkel@pixmap.fun">chninkel@pixmap.fun</a>;

  return (
    <div className="content">
      <h1>{t`Welcome to Pixmap!`}</h1>
      <h2>{t`About Pixmap`}</h2>
      <MarkdownFormatter enableFirstParagraph={true} text={
        t`Place color pixels on a large canvas with other players online!
        Our main canvas is a huge worldmap. You can place wherever you like, but you will have to wait a specific cooldown between pixels.
        You can check out the cooldown and requirements on the Canvas Selection menu (globe button on top).
        Some canvases have a different cooldown for replacing a user-set pixels than placing on a unset pixel.
        For example, 1s/2s (or "1/2 cd") means "1s" is the cooldown on never-placed pixels (also called "virign pixels"), and "2s" is the cooldown on already placed pixels.
      ...
      <h3>{t`Rules`}</h3>
      <p>
        {t`You can find the rules in the User menu.`}
        <br/>
        {t`Click `}<ActionButtonWrapper><MdPerson /></ActionButtonWrapper>{t` then "Rules"`}
      </p>
      <h3>{t`Socials`}</h3>
      <MarkdownFormatter enableFirstParagraph={true} text={
        t`Discord (recommended): [https://dsc.gg/pixmap](./guilded)
        Original work on [git.pixelplanet.fun](https://git.pixelplanet.fun)
        Source on [git.pixmap.fun](https://git.pixmap.fun)`
      }/>
      <h3>{t`I am banned`}</h3>
      <div>
      ...
      <div>
        ...
        <p>PixMap - Copyright (C) 2023, 2024 Le Chninkel chninkel@pixmap.fun | pixelplanet - Copyright (C) 2020, 2024 hf hf@pixelplanet.fun</p>
      </div>
    </div>
  );
};
