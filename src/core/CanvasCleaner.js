...
const METHODS = {
  ...
  makeblank: (xc, yc, clrIgnore, canvasCleaner) => {
    const pxl = canvasCleaner.getPixelInChunkArea(xc, yc);
    if (pxl === 2 || pxl === 1) {
  ...
  replacecolor: (xc, yc, clrIgnore, canvasCleaner) => {
    const pxl = canvasCleaner.getPixelInChunkArea(xc, yc);
    if (pxl === null) {
      return null;
    }
    // Get source and target colors from canvasCleaner instance
    const { sourceColor, targetColor } = canvasCleaner;
    if (sourceColor !== undefined && targetColor !== undefined) {
      if (pxl === sourceColor) {
        // Log first few replacements to confirm it's working
        if (canvasCleaner.pxlCleaned < 5) {
          canvasCleaner.logger(`Replacing pixel at (${xc},${yc}) from color ${sourceColor} to ${targetColor}`);
        }
        return targetColor;
      }
    }
...
class CanvasCleaner {
  ...
  methodName;
  // colors for replacecolor method
  sourceColor;
  targetColor;
  // 3x3 canvas area
  // [
  ...
  set(canvasId, x, y, u, v, methodName, cIter = 0, sourceColor = undefined, targetColor = undefined) {
    ...
    if (methodName === 'replacecolor') {
      if (sourceColor === undefined || targetColor === undefined) {
        const str = 'Source and target colors required for replacecolor method';
        this.logger(str);
        return str;
      }
      if (sourceColor === targetColor) {
        const str = 'Source and target colors cannot be the same';
        this.logger(str);
        return str;
      }
    }
    ...
    this.tick = setTimeout(this.cleanChunk, 500);
    // eslint-disable-next-line max-len
    let logMessage = `Start Cleaning on #${canvas.ident},${this.x},${this.y} till #${canvas.ident},${this.u},${this.v} with method ${methodName}`;
    if (methodName === 'replacecolor') {
      logMessage += ` (replacing color ${sourceColor} with ${targetColor})`;
    }
