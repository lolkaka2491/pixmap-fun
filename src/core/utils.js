...
/** Gets the User ID from a Markdown formatted ping.
 * The format is typically `@[Username](UserId)`.
 * @param {string} mdUserLink - The Markdown user ping.
 * @returns {string} The user ID extracted from the Markdown link.
 * @example '@[Username](12345)' returns '12345'
 */
export async function getUserIdFromMd(mdUserLink) {
...
/** Gets the Username from a Markdown formatted ping.
 * The format is typically `@[Username](UserId)`.
 * @param {string} mdUserLink - The Markdown user ping.
 * @returns {string} The user ID extracted from the Markdown link.
 * @example '@[Username](12345)' returns 'Username'
 */
export async function getUsernameFromMd(mdUserLink) {
  let mdUser = mdUserLink.trim();
  if (mdUser[0] === '@') {
    mdUser = mdUser.substring(1);
    if (mdUser[0] === '[') {
      // Trims to what is inbetween the highest "parent" square brackets. This may include nested square brackets
      // Note: The last index does not need to be subtracted by 1 due to The Fencepost Problem
      mdUser = mdUser.substring(1, mdUser.lastIndexOf(']')).trim();
    }
  }
  return mdUser;
}