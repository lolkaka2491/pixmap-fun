...
class PixelTransferController {
  ...
  async requestFromQueue() {
    ...
    // For user id 1, don't block other requests - allow concurrent processing
    if (userId !== 1) {
    ...
    switch (retCode) {
      ...
      case 16:
        ...
        // eslint-disable-next-line max-len
        msg = t`Didn't get an answer from pixmap. Try to refresh if this problem persists.`;
        break;
      case 17:
        errorTitle = t`Email Not Verified`;
        msg = t`You need to verify your email address to place pixels`;
        break;
      case 18:
        errorTitle = t`Banned`;
        try {
          const response = await fetch('/api/idbaninfo');
          
          if (response.ok) {
            const data = await response.json();
            if (data.moderator) {
              const formattedDuration = largeDurationToString(data.duration);
              msg = t`You are banned from placing pixels. Reason: ${data.reason}. Moderator: ${data.moderator}. Since: ${data.since} For: ${formattedDuration}. Remaining: ${formattedDuration}.`;
            } else {
              msg = t`You are banned from placing pixels. Please contact an admin for more information.`;
            }
          } else {
            msg = t`You are banned from placing pixels. Please contact an admin for more information.`;
          }
        ...
        break;
      case 19:
        errorTitle = t`Dotting isn't Allowed`;
        msg = t`Dotting is bannable now, Please don't dot or you might be banned.`;
        break;    
      default:
        errorTitle = t`Weird`;
    ...
    // Different delay logic for user ID 1 (unlimited user)
    const delay = (userId === 1) ? 0 : 100;
    if (this.pixelQueue.length > 0) {
      setTimeout(this.requestFromQueue, delay);
    } else {
      store.dispatch(setPixelsFetching(false));
    }
  }
  ...
  tryPlacePixel(
    ...
    const isFetching = this.store.getState().fetching.fetchingPixel;
    
    // For user ID 1, allow unlimited concurrent requests
    if (!isFetching || userId === 1) {
      this.requestFromQueue();
    }
  }

  async getBanDetails(userId) {
    try {
      const response = await fetch('/api/idbaninfo', {
        method: 'POST',
        headers: {
