...
class Renderer2D extends Renderer {
  ...
  updateView(view, origin) {
    ...
    if (scale) {
      ...
      if (origin) {
        ...
        // Store the new viewscale before clamping for legacy zoom
        const newViewscale = scale;
        // clamp to 1.0 (only when origin is given, so not on phones)
        // but only if we're not in legacy zoom mode to avoid sudden jumps
        const { legacyZoom } = state.gui;
        if (!legacyZoom) {
  ...
  renderChunks(
    ...
    // Initialize heatmap lazily only when enabled
    if (state.gui.showHeatmap && this.chunkLoader) {
      pixelHeatmap.initializeWithCanvas(this.chunkLoader, canvasSize);
    }
    ...
    // scale
    context.save();
    // Use black background when heatmap is enabled
    context.fillStyle = '#C4C4C4';
  ...
  renderMain(
    ...
    // No throttling at any zoom level
    if (!this.forceNextRender && this.forceNextSubrender) {
      this._lastSubrenderAt = performance.now();
    }
