...
export function dehydrateCanvasToken(token) {
  // token is ASCII string like "userId.ts.sig"
  const nonce = Math.floor(Math.random() * 256);
  const tokenBuf = Buffer.from(token, 'utf8');
  const obf = Buffer.allocUnsafe(tokenBuf.length);
  for (let i = 0; i < tokenBuf.length; i += 1) {
    obf[i] = tokenBuf[i] ^ ((nonce + i) & 0xFF);
  }
  const buffer = Buffer.allocUnsafe(1 + 1 + 2 + obf.length);
  buffer.writeUInt8(CANVAS_TOKEN_OP, 0);
  buffer.writeUInt8(nonce, 1);
  buffer.writeUInt16BE(obf.length, 2);
  obf.copy(buffer, 4);
  return buffer;
}
