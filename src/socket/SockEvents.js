...
class SocketEvents extends EventEmitter {
   ...
   * @param messageId Database message ID for reactions
   */
  ...
  /*
   * Force refresh chat for all users in a channel
   * @param channelId numerical id of chat channel
   * @param purgedMessageIds array of message IDs that were purged
   */
  broadcastChatRefresh(channelId, purgedMessageIds = []) {
    this.emit('chatRefresh', channelId, purgedMessageIds);
  }

  /*
   * broadcast announcement to all users
   * @param message announcement message
   * @param username moderator name
   */
  broadcastAnnouncement(message, username) {
    this.emit('announcement', { message, username });
  }

  /*
   * broadcast canvas configuration update to all users
   * @param canvasId canvas identifier
   * @param config updated canvas configuration
   */
  broadcastCanvasConfigUpdate(canvasId, config) {
    this.emit('canvasConfigUpdate', { canvasId, config });
  }

  /*
   * broadcast chat reaction to all users in channel
   * @param messageId numerical id of message
   * @param emoji emoji name
   * @param userId numerical id of user who reacted
   * @param userName name of user who reacted
   * @param channelId numerical id of channel
   * @param action 'add' or 'remove'
   */
  broadcastChatReaction(messageId, emoji, userId, userName, channelId, action) {
    this.emit('chatReaction', messageId, emoji, userId, userName, channelId, action);
  }

  /*
   * received Chat reaction on own websocket
   * @param user User Instance that sent the reaction
   * @param messageId numerical id of message
   * @param emoji emoji name
   * @param channelId numerical channel id
   */
  recvChatReaction(user, messageId, emoji, channelId) {
    this.emit('recvChatReaction', user, messageId, emoji, channelId);
  }
}
