...
import { HOLD_PAINT, PENCIL_STRATEGY } from '../core/constants';
  ...
  return (event) => {
    ...
    switch (event.key) {
      ...
      case 'Shift': {
        ...
        const { keyBindings } = state.templates;
        
        if (event.location === KeyboardEvent.DOM_KEY_LOCATION_LEFT) {
          // left shift - use custom binding
          store.dispatch(selectHoldPaint(HOLD_PAINT[keyBindings.leftShift], true));
          return;
        }
        if (event.location === KeyboardEvent.DOM_KEY_LOCATION_RIGHT) {
          // right shift - use custom binding
          store.dispatch(selectHoldPaint(HOLD_PAINT[keyBindings.rightShift], true));
          return;
        }
        return;
      }
      case ' ': // spacebar
        const state = store.getState();
        const { keyBindings } = state.templates;
        store.dispatch(selectHoldPaint(HOLD_PAINT[keyBindings.spacebar], true));
        return;
      default:
    }
