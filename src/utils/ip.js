...
/*
 * Mask IP address for moderators
 * Shows only first 2 octets for IPv4 and first 4 groups for IPv6
 * @param ip IP address to mask
 * @param isAdmin boolean - if true, return full IP; if false, return masked IP
 * @return masked or full IP address string
 */
export function maskIPForModerators(ip, isAdmin) {
  if (isAdmin) {
    return ip;
  }
  
  if (isIPv6(ip)) {
    // For IPv6, show first 4 groups and mask the rest
    const groups = ip.split(':');
    if (groups.length >= 4) {
      return `${groups.slice(0, 4).join(':')}:****:****:****:****`;
    }
    return ip; // fallback for malformed IPv6
  } else {
    // For IPv4, show first 2 octets and mask the rest
    const octets = ip.split('.');
    if (octets.length === 4) {
      return `${octets[0]}.${octets[1]}.***.***.`;
    }
    return ip; // fallback for malformed IPv4
  }
}
