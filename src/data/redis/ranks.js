...
export const TOTAL_CRANKED_KEY = 'crankt';
...
export async function getCountryRanks(start, amount) {
  ...
  // Get hourly stats for comparison
  const hourlyStats = await client.zRangeWithScores(
    HOURLY_CRANKED_KEY, 0, -1, {
      REV: true,
    });
  const hourlyMap = new Map();
  hourlyStats.forEach(({ value, score }) => hourlyMap.set(value, Number(score)));

  // Get total country stats
  const totalStats = await client.zRangeWithScores(
    TOTAL_CRANKED_KEY, 0, -1, {
      REV: true,
    });
  const totalMap = new Map();
  totalStats.forEach(({ value, score }) => totalMap.set(value, Number(score)));

  ranks = ranks.map((r) => ({
    cc: r.value,
    px: Number(r.score),
    hourlyPx: hourlyMap.get(r.value) || 0,
    totalPx: totalMap.get(r.value) || 0,
  }));
  return ranks;
}
