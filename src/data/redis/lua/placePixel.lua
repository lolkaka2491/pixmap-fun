...
-- Special case: user ID 1 gets unlimited cooldown (no rate limiting)
if ARGV[6] == "1" then
  -- Count all pixels (user ID 1 can place all pixels)
  local pxlcnt = #ARGV - 8
  -- increment pixelcount for user ID 1
  if KEYS[7] ~= 'nope' then
  ...
  -- return success with no cooldown
  ret[2] = pxlcnt
  ret[3] = 0  -- no cooldown
  ret[4] = 0  -- no added cooldown
  return ret
end
...
for c = 9,#ARGV do
  ...
  pxlcnt = pxlcnt + 1
  table.insert(allowed_indices, tostring(c - 9))
end
...
ret[3] = cd
ret[4] = pxlcd
ret[6] = table.concat(allowed_indices, ",")
return ret
