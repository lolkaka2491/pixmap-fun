...
import { getUserRanks } from './ranks';
...
export async function allowedChat(
  ...
  const country = (cc?.length !== 2) ? 'nope' : cc;
...
export async function getPixelCount(userId) {
  // getUserRanks exports [ totalPixels, dailyPixels, totalRank, dailyRank ]
  const [totalPixels] = await getUserRanks(userId);
  return totalPixels;
}