...
#  $2 = Compression level (1-9, default 9 for maximum compression)
#  $3 = Block size for squashfs (default 131072 for better compression)

# CHANGE THIS TO YOUR PATH
BACKUPROOT="/var/www/ppbc/history"
# mount options of created monthly squashfs images
MOUNT_OPTIONS="ro,defaults"
# Default compression level (1-9, 9 = maximum compression)
COMPRESSION_LEVEL=${2:-9}
# Default block size for squashfs (larger block size = better compression)
BLOCK_SIZE=${3:-131072}
...
if [ `echo "${TODAY}" | sed 's/.*\///'` == "01" ]
  then
    ...
    if [ ! -f "${SQUASH_FILE}" ]; then
      ...
      set -e
      echo "mksquashfs ${BACKUPROOT}/${PREV_YEAR}/${PREV_MONTH} ${SQUASH_FILE} -b ${BLOCK_SIZE} -comp xz -Xcompression-level ${COMPRESSION_LEVEL} -Xdict-size 64M -no-xattrs -progress -no-exports"
      /usr/sbin/mksquashfs "${BACKUPROOT}/${PREV_YEAR}/${PREV_MONTH}" ${SQUASH_FILE} -b ${BLOCK_SIZE} -comp xz -Xcompression-level ${COMPRESSION_LEVEL} -Xdict-size 64M -no-xattrs -progress -no-exports
      if [ -f "${SQUASH_FILE}" ]; then
